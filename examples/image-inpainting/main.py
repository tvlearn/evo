...
if __name__ == "__main__":
    ...
    dlog.set_handler(["psnr_mean", "psnr_median"], TextPrinter)
    ...
    # generating incomplete image and extract image patches
    ...
    if comm.rank == 0:
        ...
        isrgb = np.ndim(clean) == 3
        incomplete = clean.copy()
        incomplete[np.random.random(clean.shape) <= (float(args.percentage) / 100)] = np.nan
        print("Removed {} % of pixels".format(args.percentage))
        dlog.assign("incomplete_image", incomplete)
        OVP = MultiDimOverlappingPatches if isrgb else OverlappingPatches
        ovp = OVP(incomplete, args.patch_height, patch_width, patch_shift=1)
        Y = ovp.get().T
    else:
        Y = None
        isrgb = None
    ...
    kwargs = {"ebsc": {}, "es3c": {"to_learn": ["W", "pies", "sigma2"]}}[args.algo]
    model = MODEL(D, args.H, args.Ksize, **kwargs)
    ...
    my_data = {
        ...
        "x": np.logical_not(np.isnan(my_y)),  # not x will be reconstructed
    }
    ...
    for e in range(args.no_epochs):
        ...
        # run training epochs
        # each epoch computes data reconstructions required for M-step
        F, S_nunique, S_sub, theta = model.step(
            theta, my_suff_stat, my_data, do_reconstruction=True
        )
